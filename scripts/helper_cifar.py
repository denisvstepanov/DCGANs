...
def to_image_tensor(input_tensor, n_rows=9, n_cols=9, width=64, height=64):
    ...
    image_grid = np.zeros((n_rows * height, n_cols*width))
    for i in range(images_to_plot):
        r = i//n_rows
        c = i%n_cols
        image_grid[r * height:(r + 1) * height, c * width:(c + 1) * width] = tensor_to_plot[i, :].reshape(width, height)
    image_grid = np.stack((image_grid, image_grid, image_grid), axis=0)
